class ram_coverage extends uvm_subscriber #(ram_seq_item);
    ...
    covergroup ram_cg;
        ...
        cp_op: coverpoint tr.write {bins read = {0}; bins write = {1};}
        ...
        cp_wdata: coverpoint tr.wdata iff (tr.write) {
            bins data_zero = {8'h00}; bins data_max = {8'hFF}; bins data_etc = {[8'hAB : 8'hFE]};
        }
    ...
    function void report_phase(uvm_phase phase);
        ...
        `uvm_info("COV", $sformatf(
                  ...
                  UVM_LOW)
        `uvm_info("COV", $sformatf(
                  "  주소 : %6.2f %% (min/low/mid/high/max)", ram_cg.cp_addr.get_inst_coverage()),
                  UVM_LOW)
        `uvm_info("COV", $sformatf(
                  "  데이터 : %6.2f %% (0/FF/etc)", ram_cg.cp_wdata.get_inst_coverage()),
                  UVM_LOW)
        `uvm_info("COV", $sformatf(
                  "  동작x주소 : %6.2f %% (cross)", ram_cg.cx_op_addr.get_inst_coverage()),
                  UVM_LOW)
        `uvm_info("COV", "================================", UVM_LOW)
